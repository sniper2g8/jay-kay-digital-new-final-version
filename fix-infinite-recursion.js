...
async function fixInfiniteRecursion() {
  ...
  try {
    ...
    for (const policy of existingPolicies.rows) {
      await client.query(`DROP POLICY IF EXISTS "${policy.policyname}" ON public."appUsers"`);
    ...
    try {
      ...
      console.log(`✅ appUsers query successful! Found ${testResult.rows.length} users`);
    ...
    console.log('⚠️  Admin privileges will be handled by application logic for now');
