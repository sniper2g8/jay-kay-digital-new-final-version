...
async function analyzeAndCleanup() {
  ...
  try {
    ...
    if (stats) {
      ...
      console.log(`estimated_cost populated: ${withEstimatedCost} (${(withEstimatedCost/totalJobs*100).toFixed(1)}%)`);
      console.log(`final_cost populated: ${withFinalCost} (${(withFinalCost/totalJobs*100).toFixed(1)}%)`);
      console.log(`unit_price populated: ${withUnitPrice} (${(withUnitPrice/totalJobs*100).toFixed(1)}%)`);
      console.log(`estimate_price populated: ${withEstimatePrice} (${(withEstimatePrice/totalJobs*100).toFixed(1)}%)`);
      console.log(`estimate JSON populated: ${withEstimateJson} (${(withEstimateJson/totalJobs*100).toFixed(1)}%)`);
    }
    ...
    console.log('   2. estimate JSON column (migrated to unit_price/estimate_price)');
    console.log('\n✅ Keep:');
