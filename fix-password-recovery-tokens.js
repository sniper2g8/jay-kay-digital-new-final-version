...
async function fixPasswordRecoveryTokens() {
  ...
  try {
    ...
    console.log(
      `   Users with empty confirmation_token: ${stats.bad_confirmation_tokens}`,
    );
    console.log(
      `   Users with empty recovery_token: ${stats.bad_recovery_tokens}`,
    );
    console.log(
      `   Users with empty email_change_token: ${stats.bad_email_change_tokens}`,
    );
    ...
    `);
    console.log(
      `   ✅ Fixed ${confirmationFix.rowCount} confirmation_token fields`,
    );
    ...
    `);
    console.log(
      `   ✅ Fixed ${emailNewFix.rowCount} email_change_token_new fields`,
    );
    console.log(
      `   ✅ Fixed ${emailCurrentFix.rowCount} email_change_token_current fields`,
    );
    ...
    console.log(
      `   - Bad confirmation_token: ${finalStats.bad_confirmation_tokens} (should be 0)`,
    );
    console.log(
      `   - Bad recovery_token: ${finalStats.bad_recovery_tokens} (should be 0)`,
    );
    console.log(
      `   - Bad email_change_token: ${finalStats.bad_email_change_tokens} (should be 0)`,
    );
    console.log(
      `   - Bad phone_change_token: ${finalStats.bad_phone_change_tokens} (should be 0)`,
    );

    if (
      finalStats.bad_confirmation_tokens == 0 &&
      finalStats.bad_recovery_tokens == 0
    ) {
      console.log(
    ...
    userCheck.rows.forEach((user) => {
      console.log(
        `   ${user.email}: confirmation_token=${user.confirmation_clean ? "NULL✅" : "BAD❌"}, recovery_token=${user.recovery_clean ? "NULL✅" : "BAD❌"}`,
      );
    });

    console.log("\n🚀 Password recovery token cleanup completed!");
    console.log(
      "   Next: Test password recovery at http://localhost:3000/auth/forgot-password",
    );
  } catch (err) {
