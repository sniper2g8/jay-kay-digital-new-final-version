-- =====================================================
-- CORE BUSINESS TABLES RLS POLICIES (WITH PROPER TYPE CASTING)
-- =====================================================
...
-- Drop ALL existing policies on core tables using a more aggressive approach
-- For customers table
DO $$ 
BEGIN 
...
BEGIN 
  ...
  DROP POLICY IF EXISTS "service_role_bypass_invoices" ON invoices;
EXCEPTION WHEN undefined_object THEN 
  -- Policy does not exist, continue
END; 
$$;
...
BEGIN 
  ...
  DROP POLICY IF EXISTS "service_role_bypass_payments" ON payments;
EXCEPTION WHEN undefined_object THEN 
  -- Policy does not exist, continue
END; 
$$;
...
-- =====================================================
-- STATEMENT TABLES RLS POLICIES (WITHOUT UNNECESSARY CASTING)
-- =====================================================
...
-- Drop ALL existing policies on statement tables using a more aggressive approach
-- For customer_statement_periods table
DO $$ 
BEGIN 
...
-- For customer_statement_transactions table
DO $$ 
BEGIN 
...
-- For customer_account_balances table
DO $$ 
BEGIN 
...
USING (
  EXISTS (
    ...
    AND (
      customer_statement_periods.customer_id = auth.uid()
      OR EXISTS (
        SELECT 1 FROM "appUsers" 
        WHERE id = auth.uid() 
        AND primary_role IN ('admin', 'super_admin', 'manager', 'staff')
