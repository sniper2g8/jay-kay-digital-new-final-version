...
async function testCustomersQuery() {
  ...
  try {
    ...
    if (userResult.rows.length === 0) {
      ...
      console.log(
        `Using user: ${anyUserResult.rows[0].email} (${anyUserResult.rows[0].primary_role})`,
      );
    } else {
      console.log(
        `Using admin user: ${userResult.rows[0].email} (${userResult.rows[0].primary_role})`,
      );
    }
    ...
    try {
      ...
      console.log(
        `✅ Query successful! Found ${customersResult.rows.length} customers`,
      );
    ...
    policiesResult.rows.forEach((policy) => {
      console.log(
        `- ${policy.policyname} (${policy.cmd}): ${policy.qual || "no condition"}`,
      );
    });
