...
async function analyzeThreeTables() {
  console.log('📊 Complete Analysis of appUsers, profiles, and customers Tables\n');
  ...
  try {
    ...
    if (error) {
      ...
      appUsers.forEach(u => {
        console.log(`  - ${u.email} | ${u.name} | ${u.primary_role} | ${u.human_id}`);
      });
    }
  } catch (err) {
  ...
  try {
    ...
    if (error) {
      ...
      customers.forEach(c => {
        console.log(`  - ${c.email || 'no email'} | ${c.business_name} | ${c.contact_person || 'no contact'} | ${c.human_id}`);
      });
    }
  } catch (err) {
  ...
  try {
    const { data: appUsersEmails } = await supabase.from('appUsers').select('email');
    const { data: profilesEmails } = await supabase.from('profiles').select('email');
    const { data: customersEmails } = await supabase.from('customers').select('email');
    
    const appUsersSet = new Set((appUsersEmails || []).map(u => u.email));
    const profilesSet = new Set((profilesEmails || []).map(p => p.email));
    const customersSet = new Set((customersEmails || []).filter(c => c.email).map(c => c.email));
    ...
    // Find overlaps
    const appUsersProfilesOverlap = [...appUsersSet].filter(email => profilesSet.has(email));
    const appUsersCustomersOverlap = [...appUsersSet].filter(email => customersSet.has(email));
    
    console.log(`\nOverlaps:`);
    console.log(`appUsers ↔ profiles: ${appUsersProfilesOverlap.length} emails`);
    console.log(`appUsers ↔ customers: ${appUsersCustomersOverlap.length} emails`);
  ...
  console.log('- profiles: View/sync of appUsers for Supabase auth compatibility');
  console.log('- customers: Business entities that place orders (separate from users)');
}
