...
async function debugSupabaseStorage() {
  ...
  try {
    ...
    if (bucketsResult.rows.length === 0) {
      ...
      console.log(
        "💡 You need to create storage buckets in Supabase Dashboard",
      );
    } else {
      bucketsResult.rows.forEach((bucket) => {
        console.log(
          `  - ${bucket.name} (${bucket.is_public ? "Public" : "Private"}) - Created: ${bucket.created_at}`,
        );
      });
    }
    ...
    try {
      const fileCount = await client.query(
        "SELECT COUNT(*) as count FROM public.file_attachments",
      );
      console.log(
      ...
      if (fileCount.rows[0].count > 0) {
        ...
        sampleFiles.rows.forEach((file) => {
          console.log(
            `  - ${file.filename} (${file.entity_type}:${file.entity_id})`,
          );
