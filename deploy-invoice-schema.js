...
async function deployInvoiceSchema() {
  ...
  try {
    ...
    const schemaPath = path.join(__dirname, 'migrations', 'invoice_management_enhanced_fixed.sql');
    ...
    const lineItemColumnsResult = await client.query(`
      ...
      ORDER BY column_name
    `);
    console.log(`📋 Enhanced Line Item Columns: ${lineItemColumnsResult.rows.map(r => r.column_name).join(', ')}`);
    ...
    const paymentColumnsResult = await client.query(`
      ...
      ORDER BY column_name
    `);
    console.log(`💳 Enhanced Payment Columns: ${paymentColumnsResult.rows.map(r => r.column_name).join(', ')}`);
    ...
    `);
    console.log(`⚡ Invoice Triggers: ${triggersResult.rows.length} triggers created`);
    ...
    console.log('   ✅ New tables created: 4 (templates, allocations, history, recurring)');
    ...
    if (error.message.includes('permission denied')) {
      console.error('\n💡 Permission Issue:');
      console.error('   - Make sure you are using the SERVICE_ROLE_KEY, not ANON_KEY');
    ...
    if (error.message.includes('does not exist')) {
      ...
      console.error('   - Ensure customer_statements_production.sql was executed first');
      console.error('   - Check that required tables (customers, jobs, appUsers) exist');
    }
