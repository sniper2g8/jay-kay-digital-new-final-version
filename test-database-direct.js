...
async function testDirectDatabaseAccess() {
  console.log('🎯 Testing Enhanced Invoice Management via Direct Database Access...\n');
  ...
  try {
    ...
    console.log(`✅ Invoice Templates: ${templatesResult.rows.length} templates`);
    templatesResult.rows.forEach(row => {
      console.log(`   📋 Template: "${row.template_name}" (${row.count} records)`);
    });
    ...
    const newTables = ['payment_allocations', 'invoice_status_history', 'recurring_invoices'];
    ...
    `);
    console.log(`✅ Invoice-related Triggers: ${triggersResult.rows.length} active`);
    ...
    `);
    console.log(`✅ RLS Policies: ${policiesResult.rows.length} policies active`);
    ...
    console.log(`✅ Customers Available: ${customersResult.rows[0].count} customers for invoice creation`);
