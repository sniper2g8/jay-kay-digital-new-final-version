...
async function debugRLSIssue() {
  ...
  try {
    ...
    usersCheck.rows.forEach((user) => {
      console.log(
        `  ${user.email} (${user.primary_role}) - ID: ${user.id.substring(0, 8)}...`,
      );
    });
    ...
    try {
      await client.query(
        'DROP POLICY IF EXISTS "auth_users_select_appUsers" ON public."appUsers"',
      );
    ...
    try {
      await client.query(
        'DROP POLICY IF EXISTS "admin_full_access_appUsers" ON public."appUsers"',
      );
