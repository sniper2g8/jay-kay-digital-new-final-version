...
async function fixStoragePolicies() {
  ...
  try {
    ...
    for (const policy of existingPolicies.rows) {
      await client.query(
        `DROP POLICY IF EXISTS "${policy.policyname}" ON storage.objects`,
      );
    ...
    for (const policy of attachmentPolicies.rows) {
      await client.query(
        `DROP POLICY IF EXISTS "${policy.policyname}" ON public.file_attachments`,
      );
    ...
    console.log(
      `✅ job-files bucket exists: ${testResult.rows[0].bucket_count > 0 ? "Yes" : "No"}`,
    );
    ...
    console.log(
      "✅ file_attachments table is accessible to authenticated users",
    );
