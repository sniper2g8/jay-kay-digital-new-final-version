...
async function testConnection() {
  try {
    ...
    console.log('Schema query result:', tablesError ? 'Error: ' + tablesError.message : 'Success');
    
    // Test 3: Try customers table with more explicit error handling
    console.log('\n3. Testing customers table access...');
    
    try {
      const { data: customers, error: customersError, status, statusText } = await supabase
        .from('customers')
        .select('customer_human_id')
        .limit(1);
      ...
      if (customersError) {
        ...
        if (customersError.message && customersError.message.includes('row level security')) {
          console.log('*** This appears to be a Row Level Security (RLS) issue ***');
          console.log('*** The customers table has RLS enabled but no policies allow anonymous access ***');
        }
