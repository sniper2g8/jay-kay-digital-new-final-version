...
async function diagnosePwdRecovery() {
  try {
    ...
    console.log('   Protocol:', redirectUrl.startsWith('http') ? '✅ Valid' : '❌ Invalid');
    
    // Check 5: Common issues checklist
    console.log('\n5️⃣ Common Issues Checklist:');
    console.log('   ❓ Check these in your Supabase Dashboard:');
    console.log('   1. Authentication > Settings > Site URL includes http://localhost:3000');
    console.log('   2. Authentication > Settings > Redirect URLs includes http://localhost:3000/**');
    console.log('   3. Authentication > Settings > Email templates are configured');
    console.log('   4. Authentication > Settings > SMTP settings (if using custom email)');
    console.log('   5. Check Auth logs in Supabase Dashboard for detailed error info');
