...
async function createAndConfigureBucket() {
  console.log(
    "🪣 Creating and configuring job-files bucket with admin privileges...",
  );
  ...
  try {
    ...
    if (uploadError) {
      ...
      const { data: downloadData, error: downloadError } =
        await supabaseAdmin.storage.from("job-files").download(testPath);

      if (downloadError) {
        console.error("❌ Error downloading test file:", downloadError);
      } else {
        console.log(
          "✅ Test file downloaded successfully, size:",
          downloadData.size,
          "bytes",
        );
      }
