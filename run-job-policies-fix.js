...
async function fixJobSubmissionPolicies() {
  ...
  try {
    ...
    if (
      customersError &&
      !customersError.message.includes("permission denied")
    ) {
    ...
    if (testJobError) {
      ...
      if (testJobError.message.includes("permission denied")) {
        ...
        console.log(
          "3. Create storage policies as described in STORAGE_POLICIES_GUIDE.md",
        );
      }
    } else {
    ...
    console.log(
      '2. Create policies for the "job-files" bucket as described in STORAGE_POLICIES_GUIDE.md',
    );
  } catch (error) {
    ...
    console.log("2. Paste and execute in your Supabase SQL editor");
    console.log(
      "3. Create storage policies manually as described in STORAGE_POLICIES_GUIDE.md",
    );
  }
}
