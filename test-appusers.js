...
async function testAppUsersQuery() {
  try {
    ...
    const { data: appUsers, error: appUsersError, status } = await supabase
      .from('appUsers')
      .select('*')
      .limit(1);
    ...
    if (appUsers && appUsers.length > 0) {
      ...
      if (schemaError) {
        ...
        const columnTests = ['id', 'email', 'primary_role', 'status', 'human_id', 'name'];
    ...
    if (roleError) {
      ...
      if (roleError.code === 'PGRST116') {
        ...
        console.log('✗ Query structure issue - this is likely the login problem!');
      }
    } else {
    ...
    if (anonError) {
      ...
      console.log('This suggests RLS is active - users need to be authenticated to query appUsers');
    } else {
