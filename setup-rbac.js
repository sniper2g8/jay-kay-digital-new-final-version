  ...
  try {
    ...
    const defaultRoles = [
      { 
        ...
        permissions: JSON.stringify(['jobs.create', 'jobs.read.own', 'customers.read.own', 'invoices.read.own'])
      },
      { 
        id: 'staff', 
        name: 'Staff', 
        description: 'Staff member with operational access',
        permissions: JSON.stringify(['jobs.read', 'jobs.update', 'customers.read', 'inventory.read'])
      },
      { 
        id: 'manager', 
        name: 'Manager', 
        description: 'Manager with extended access',
        permissions: JSON.stringify(['jobs.create', 'jobs.read', 'jobs.update', 'customers.read', 'customers.update', 'inventory.read', 'inventory.update', 'reports.operational'])
      },
      { 
        id: 'admin', 
        name: 'Admin', 
        description: 'Administrator with full operational access',
        permissions: JSON.stringify(['jobs.*', 'customers.*', 'invoices.*', 'payments.*', 'inventory.*', 'reports.*'])
      },
      { 
        id: 'super_admin', 
        name: 'Super Admin', 
    ...
    if (signInError) {
      ...
      if (appUserError) {
        console.error('❌ Error creating appUser record:', appUserError.message);
      } else {
    ...
    if (appUsersError) {
      ...
      appUsersVerify.forEach(user => {
        console.log(`   - ${user.email}: ${user.primary_role} (${user.status})`);
      });
    }
