...
async function cleanupRedundantColumns() {
  ...
  try {
    ...
    if (updateError) {
      ...
      console.log('✅ Consolidated estimated_cost into final_cost where needed');
    }
    ...
    if ((withNewColumns?.length || 0) > 0) {
      ...
      console.log('   - estimate (JSON) column - data migrated to unit_price/estimate_price');
      console.log('   - estimated_cost column - consolidated with final_cost');
      console.log('\n🚀 Ready to proceed with column removal!');
    } else {
      console.log('⚠️  New pricing columns not populated - run migration first');
    }
