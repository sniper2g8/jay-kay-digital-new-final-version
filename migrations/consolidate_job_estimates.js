...
async function consolidateJobEstimates() {
  ...
  try {
    ...
    console.log("\n📊 Analysis Summary:");
    console.log(
      `  Jobs with estimated_cost: ${pricingAnalysis.has_estimated_cost}/${jobs.length}`,
    );
    console.log(
      `  Jobs with final_cost: ${pricingAnalysis.has_final_cost}/${jobs.length}`,
    );
    console.log(
      `  Jobs with estimate JSON: ${pricingAnalysis.has_estimate_json}/${jobs.length}`,
    );
    console.log(
      `  Jobs missing all pricing: ${pricingAnalysis.missing_all_pricing}/${jobs.length}`,
    );
    ...
    Object.entries(estimateStructures).forEach(([structure, data]) => {
      ...
      console.log(
        `    Jobs: ${data.jobs.slice(0, 3).join(", ")}${data.jobs.length > 3 ? "..." : ""}`,
      );
    });
    ...
    console.log(
      `  💰 Pricing data coverage: ${(((jobs.length - pricingAnalysis.missing_all_pricing) / jobs.length) * 100).toFixed(1)}%`,
    );
...
async function generateImprovedStatsLogic(jobs, analysis, structures) {
  ...
  for (const job of jobs) {
    ...
    if (job.final_cost && parseFloat(job.final_cost) > 0) {
      ...
      if (job.estimate.total && parseFloat(job.estimate.total) > 0) {
        ...
        job.estimate.total_price &&
        parseFloat(job.estimate.total_price) > 0
      ) {
        jobValue = parseFloat(job.estimate.total_price);
      } else if (
        job.estimate.totalPrice &&
        parseFloat(job.estimate.totalPrice) > 0
      ) {
    ...
    if (jobValue > 0) {
      ...
      if (
        !Number.isFinite(totalEstimatedValue) ||
        !Number.isFinite(totalFinalValue)
      ) {
        console.log(
          `❌ Invalid value detected for job ${job.jobNo}: jobValue=${jobValue}, estimated=${job.estimated_cost}, totalEst=${totalEstimatedValue}, totalFinal=${totalFinalValue}`,
        );
      }
    }
  }
  ...
  console.log(
    `    Total estimated value: $${(Number.isFinite(totalEstimatedValue) ? totalEstimatedValue : 0).toFixed(2)}`,
  );
  console.log(
    `    Total final value: $${(Number.isFinite(totalFinalValue) ? totalFinalValue : 0).toFixed(2)}`,
  );
  console.log(
    `    Average job value: $${jobsWithPricing > 0 ? (totalFinalValue / jobsWithPricing).toFixed(2) : "0.00"}`,
  );
}
...
export const useJobStatsImproved = () => {
  ...
  console.log(
    "📝 You should update src/lib/hooks/useJobs.ts with the improved logic",
  );
...
async function verifyConsolidation() {
  ...
  console.log(
    `  Jobs with any pricing data: ${jobsWithAnyPricing}/${jobs.length} (${((jobsWithAnyPricing / jobs.length) * 100).toFixed(1)}%)`,
  );
  console.log(
    `  Total business value: $${totalValueFromAllSources.toFixed(2)}`,
  );
  console.log(
    `  Average job value: $${(totalValueFromAllSources / jobsWithAnyPricing).toFixed(2)}`,
  );
  ...
  if (jobsWithAnyPricing >= jobs.length * 0.8) {
    ...
    console.log(
      "⚠️  Some jobs missing pricing data - consider data entry improvements",
    );
  }
}
