  ...
  try {
    ...
    lines.forEach((line, index) => {
      const importMatch = line.match(
        /^import\s+(?:type\s+)?(?:{([^}]+)}|(\w+)|(\*\s+as\s+\w+))\s+from\s+['"][^'"]+['"];?\s*$/,
      );
      if (importMatch) {
        if (importMatch[1]) {
    ...
    const codeWithoutImports = lines
      .filter(
        (_, index) =>
          !Array.from(imports.values()).some((imp) => imp.line === index),
      )
      .join("\n");
    ...
    lines.forEach((line, index) => {
      const importInfo = Array.from(imports.values()).find(
        (imp) => imp.line === index,
      );
      ...
      if (importInfo && !processedLines.has(index)) {
        ...
        if (importMatch && importMatch[1].startsWith("{")) {
          ...
          if (namedImports.length > 0) {
            newLines.push(
              `import { ${namedImports.join(", ")} } from ${importMatch[2]}`,
            );
          }
        } else {
    ...
    if (newContent !== content) {
      ...
      console.log(
        `✅ Cleaned unused imports in: ${path.relative(process.cwd(), filePath)}`,
      );
      return true;
    }
...
function main() {
  ...
  if (changedCount > 0) {
    ...
    console.log(
      "💡 Tip: Run your linter/formatter to ensure code style consistency.",
    );
  } else {
