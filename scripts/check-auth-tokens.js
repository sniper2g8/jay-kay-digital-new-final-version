...
async function checkAuthTokens() {
  ...
  try {
    ...
    if (error) {
      console.warn(
        "⚠️  Could not execute detailed token analysis:",
        error.message,
      );
      console.log(
        "💡 This might be due to permissions. Trying simpler query...",
      );
      ...
      console.log(
        `✅ Database connection working. Found ${simpleData.count} appUsers.`,
      );
      console.log(
        "💡 The auth token issue might be resolved if you can see this message.",
      );
      return;
    }
    ...
    if (data && data.length > 0) {
      ...
      console.log(
        `   Empty Email Change Tokens: ${stats.empty_email_change_new_tokens}`,
      );
      console.log(
      ...
      console.log(
        `   NULL Confirmation Tokens: ${stats.null_confirmation_tokens}`,
      );
