...
async function fixFirebaseMigration() {
  ...
  try {
    ...
    try {
      const testResult = await client.query('SELECT COUNT(*) FROM auth.users LIMIT 1');
    ...
    try {
      ...
      `);
      console.log(`✓ Updated ${confirmationResult.rowCount} confirmation_token empty strings to NULL`);
      ...
      `);
      console.log(`✓ Updated ${recoveryResult.rowCount} recovery_token empty strings to NULL`);
      ...
      `);
      console.log(`✓ Updated ${emailNewResult.rowCount} email_change_token_new empty strings to NULL`);
      ...
      `);
      console.log(`✓ Updated ${emailCurrentResult.rowCount} email_change_token_current empty strings to NULL`);
      ...
      `);
      console.log(`✓ Updated ${phoneResult.rowCount} phone_change_token empty strings to NULL`);
      
    } catch (error) {
      if (error.message.includes('insufficient privilege')) {
        console.log('⚠ Cannot update auth.users tokens - insufficient privileges (this is normal for hosted Supabase)');
        console.log('📧 Contact Supabase support to fix empty string token columns in auth.users');
      } else {
    ...
    try {
      ...
      await client.query('DROP POLICY IF EXISTS "Allow anonymous read access to appUsers" ON public."appUsers"');
      await client.query('DROP POLICY IF EXISTS "Allow authenticated read access to appUsers" ON public."appUsers"');
      ...
      await client.query('GRANT SELECT ON information_schema.table_constraints TO anon');
      console.log('✓ Information schema permissions granted');

    } catch (error) {
      console.log('⚠ Permission error (may be normal for hosted Supabase):', error.message);
    }
    ...
    try {
      const testResult = await client.query('SELECT count(*) as appUsers_count FROM public."appUsers"');
      console.log(`✓ appUsers table accessible - found ${testResult.rows[0].appusers_count} users`);
    } catch (error) {
      console.log('⚠ Test query failed:', error.message);
    }

    console.log('\n🎉 Firebase migration auth fix completed!');
    console.log('📋 Summary: Empty string tokens converted to NULL, permissions updated');
