  ...
  try {
    ...
    if (appUsersError) {
      ...
      if (appUsersError.code === '42501') {
        console.log('💡 Need to run fix-login-permissions.sql in Supabase dashboard');
        return false;
      }
    } else {
    ...
    if (roleError) {
      if (roleError.code === 'PGRST116') {
        console.log('✅ useUserRole query structure works (no matching user found - expected)');
      } else {
    ...
    console.log('📝 If all tests pass, login should work without database errors.');
  ...
  try {
    ...
    if (userRoleError) {
      if (userRoleError.code === 'PGRST116') {
        console.log('✅ useUserRole query works (user not found - normal for test)');
