...
async function testAuthentication() {
  try {
    ...
    if (profilesError) {
      ...
      console.log('✅ Profiles view accessible:', profiles?.length || 0, 'records visible');
    }
    ...
    if (appUsersError) {
      ...
      console.log('✅ appUsers table accessible:', appUsers?.length || 0, 'records visible');
      if (appUsers && appUsers.length > 0) {
        console.log('   Sample user:', appUsers[0].email, '-', appUsers[0].primary_role);
      }
    }
    ...
    if (customersError) {
      ...
      console.log('✅ Customers with relationships:', customers?.length || 0, 'records');
      customers?.forEach(customer => {
        const contact = customer.appUsers;
        console.log(`   ${customer.business_name} -> Contact: ${contact?.name || 'None'} (${contact?.email || 'N/A'})`);
      });
    }
    ...
    if (authError) {
      console.log('✅ Auth users table properly restricted:', authError.message);
    } else {
