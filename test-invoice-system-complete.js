...
async function testInvoiceSystemFull() {
  ...
  try {
    ...
    if (templatesError) {
      ...
      console.log(`✅ Templates accessible - ${templates?.length || 0} templates found`);
      if (templates && templates.length > 0) {
        console.log(`   📋 Default template: "${templates.find(t => t.is_default)?.template_name}"`);
      }
    }
    ...
    const { data: invoices, error: invoicesError } = await supabase
      .from('invoices')
      .select('id, invoice_status, invoice_date, terms_days, template_id, customer_id')
      .limit(5);
    
    if (invoicesError) {
      console.log('❌ Invoices error:', invoicesError.message);
    } else {
      console.log(`✅ Enhanced invoices accessible - ${invoices?.length || 0} records`);
    }
    ...
    if (allocationsError) {
      ...
      console.log(`✅ Payment allocations accessible - ${allocations?.length || 0} records`);
    }
    ...
    if (historyError) {
      ...
      console.log(`✅ Invoice status history accessible - ${history?.length || 0} records`);
    }
    ...
    if (lineItemsError) {
      ...
      console.log(`✅ Enhanced line items accessible - ${lineItems?.length || 0} records`);
    }
    ...
    if (paymentsError) {
      ...
      console.log(`✅ Enhanced payments accessible - ${payments?.length || 0} records`);
    }
    ...
    if (customersError) {
      ...
      console.log(`✅ Customers accessible for invoices - ${customers?.length || 0} customers`);
    ...
    if (recurringError) {
      ...
      console.log(`✅ Recurring invoices accessible - ${recurring?.length || 0} records`);
    }
