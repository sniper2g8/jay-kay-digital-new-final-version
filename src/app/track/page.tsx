...
export default function TrackJobPage() {
    ...
    try {
      ...
      if (fetchError) {
        if (fetchError.code === "PGRST116") {
          setError(
            "Job not found. Please check your tracking number and try again.",
          );
        } else {
  ...
  const currentStatus =
    jobInfo && jobInfo.status
      ? statusConfig[
          jobInfo.status.toLowerCase() as keyof typeof statusConfig
        ] || statusConfig.pending
      : null;
  const currentPriority =
    jobInfo && jobInfo.priority
      ? priorityConfig[
          jobInfo.priority.toLowerCase() as keyof typeof priorityConfig
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Enter your job tracking number to check the current status and
              progress of your printing job.
            </p>
          </div>
            ...
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                <Input
                  ...
                  className="text-lg"
                />
                <Button onClick={searchJob} disabled={loading} className="px-6">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div>
                    <div className="flex justify-between text-sm text-gray-600 mb-2">
                      ...
                      <span>
                        {Math.round(
                          calculateProgress(jobInfo.status || "pending"),
                        )}
                        % Complete
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 rounded-full h-3">
                      <div
                        ...
                        style={{
                          width: `${calculateProgress(jobInfo.status || "pending")}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                        ...
                        <div className="space-y-2 text-sm">
                          ...
                          {jobInfo.estimatedCompletion && (
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">
                                {formatDate(jobInfo.estimatedCompletion)}
                              </span>
                            </div>
                          )}
                        </div>
                          ...
                          <div className="space-y-2 text-sm">
                            {jobInfo.totalCost && (
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium">
                                  {formatCurrency(jobInfo.totalCost)}
                                </span>
                              </div>
                            )}
                            {jobInfo.paidAmount && (
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium text-green-600">
                                  {formatCurrency(jobInfo.paidAmount)}
                                </span>
                              </div>
                            )}
                          </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mb-3">
                          Use this QR code to quickly access job status on any
                          device
                        </p>
                      </div>
              ...
              <Card>
                <CardContent className="pt-6">
                  <div className="text-center space-y-2">
                    ...
                    <p className="text-sm text-gray-600">
                      If you have any questions about your job, please contact
                      us:
                    </p>
                    <div className="flex justify-center space-x-6 text-sm">
                      <span className="text-blue-600">📞 +232 34 788711</span>
                      <span className="text-blue-600">
                        ✉️ info@jaykaypress.com
                      </span>
                    </div>
                  </div>
                </CardContent>
