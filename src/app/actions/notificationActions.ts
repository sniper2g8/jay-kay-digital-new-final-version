  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === '42501') {
        ...
        console.error("Permission denied: RLS policies may not be configured for notifications table");
        
        // Try a more basic query to see if the table is accessible at all
        const { data: basicData, error: basicError } = await supabase
          .from("notifications")
          .select("id, title, message, created_at, read_at, email_sent, sms_sent")
          .limit(1);
        ...
        if (basicError) {
          ...
          return { 
            success: false,
            error: 'Permission denied - RLS policies need to be configured for notifications table',
            code: error.code,
            details: 'Row Level Security is enabled on the notifications table but no policies allow user access',
            notifications: [],
            stats: { total: 0, unread: 0, email_sent: 0, sms_sent: 0 }
          };
        } else {
          ...
          return { 
            success: false,
            error: 'Permission denied - User-specific notifications access not configured',
            code: error.code,
            details: 'RLS policies need to be updated to allow users to access their own notifications',
            notifications: [],
            stats: { total: 0, unread: 0, email_sent: 0, sms_sent: 0 }
          };
        }
