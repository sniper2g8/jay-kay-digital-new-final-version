    ...
    <div className="h-32 flex items-end justify-between space-x-1">
      {data.map((item, index) => (
        <div key={index} className="flex-1 flex flex-col items-center">
          ...
          ></div>
          <span className="text-xs text-gray-500 mt-1 transform -rotate-45 origin-left">{item.label}</span>
        </div>
      ))}
    </div>
  </div>
...
export default function AnalyticsPage() {
  ...
  const fetchRevenueAnalytics = async () => {
    try {
      ...
      const totalRevenue = (jobsData || []).reduce((sum, job) => sum + (job.final_cost || 0), 0);
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        monthlyRevenueData.push({
          ...
          revenue: monthPayments.reduce((sum, payment) => sum + (payment.amount || 0), 0)
        });
      }
  ...
  const fetchCustomerAnalytics = async () => {
    try {
      ...
      const customerSpending = (jobsData || []).reduce((acc, job) => {
        if (job.customer_id) {
          acc[job.customer_id] = (acc[job.customer_id] || 0) + (job.final_cost || 0);
        }
        return acc;
      }, {} as Record<string, number>);
      ...
      const topCustomers = Object.entries(customerSpending)
        ...
        .map(([customerId, totalSpent]) => {
          ...
          const jobCount = (jobsData || []).filter(j => j.customer_id === customerId).length;
          return {
            name: customer?.business_name || customer?.contact_person || 'Unknown Customer',
            totalSpent,
            jobCount
          };
        });
  ...
  const fetchInventoryAnalytics = async () => {
    try {
      const { data: inventoryData, error: inventoryError } = await supabase
        .from('inventory')
        .select('id, current_stock, unit_cost, minimum_stock, category, item_name, status');
      ...
      const lowStockItems = (inventoryData || []).filter(item => 
        (item.current_stock || 0) <= (item.minimum_stock || 0) || item.status === 'low_stock'
      ).length;

      // Top moving items
      const itemMovements = (movementsData || []).reduce((acc, movement) => {
        acc[movement.inventory_id || ''] = (acc[movement.inventory_id || ''] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);
      ...
      const categoryValues = (inventoryData || []).reduce((acc, item) => {
      ...
      const inventoryValueData = Object.entries(categoryValues).map(([category, value]) => ({
        category,
        value
      }));
  ...
  const fetchFinancialAnalytics = async () => {
    try {
      ...
      const totalPayments = (paymentsData || []).reduce((sum, payment) => 
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-1 text-xs text-gray-500">
              ...
              <span>{formatPercentage(analytics.revenue.revenueGrowth)} from last month</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500">
              {analytics.jobs.completedJobs} of {analytics.jobs.totalJobs} jobs completed
            </p>
          </CardContent>
        </Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold">{analytics.jobs.totalJobs}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Completed</span>
                    <span className="font-bold text-green-600">{analytics.jobs.completedJobs}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span>Pending</span>
                    <span className="font-bold text-yellow-600">{analytics.jobs.pendingJobs}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="text-center p-6 bg-blue-50 rounded-lg">
                    ...
                    <div className="text-blue-800 font-medium">Total Customers</div>
                  </div>
                  <div className="text-center p-6 bg-green-50 rounded-lg">
                    <div className="text-3xl font-bold text-green-600">
                      {analytics.customers.newCustomers}
                    </div>
                    <div className="text-green-800 font-medium">New This Month</div>
                  </div>
                </div>
              </CardContent>
            </Card>
