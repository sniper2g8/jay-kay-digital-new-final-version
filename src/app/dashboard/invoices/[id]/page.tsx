...
interface InvoiceWithDetails extends Invoice {
  ...
  // Add missing fields from the actual database
...
function InvoiceDetailContent() {
  ...
  const fetchInvoiceDetails = useCallback(async () => {
    try {
      ...
      const itemsError = itemsResponse.ok ? null : new Error('Failed to fetch items');
      ...
      const invoiceWithDetails: InvoiceWithDetails = {
        ...
        customerName: invoiceData.customers?.business_name || invoiceData.customerName || '',
  ...
  const handleStatusUpdate = async (newStatus: string) => {
    ...
    try {
      ...
      setInvoice(prev => prev ? { ...prev, invoice_status: newStatus } : null);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const { error: paymentError } = await supabase
        .from('payments')
        .insert({
          human_id: invoice.customer?.id || invoice.human_id,
  ...
  const itemsSubtotal = invoice.invoice_items?.reduce((sum, item) => sum + Number(item.total_price), 0) || 0;
  const displaySubtotal = itemsSubtotal || Number(invoice.subtotal) || Number(invoice.total) || 0;
        ...
        <div className="mb-8">
                  ...
                  <Dialog open={showPaymentDialog} onOpenChange={setShowPaymentDialog}>
                    ...
                    <DialogContent className="sm:max-w-md">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Record a payment for {invoice.invoiceNo || `Invoice #${invoice.id.slice(0, 8)}`}
                        </DialogDescription>
                      </DialogHeader>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-2">
                ...
                >
                  ...
                  <span className="ml-1 capitalize">{invoice.invoice_status || 'draft'}</span>
                </Badge>
            ...
            <div className="text-right">
              <div className="text-3xl font-bold">{formatCurrency(displayTotal)}</div>
              {amountPaid > 0 && (
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {invoice.invoice_items?.length || 0} item(s) on this invoice
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  {amountPaid > 0 ? (
                    ...
                    <div className="text-center py-8">
                      ...
                      <Button 
                        size="sm"
                        onClick={() => {
                          setPaymentForm(prev => ({ ...prev, amount: displayTotal.toString() }));
                          setShowPaymentDialog(true);
                        }}
                      >
                        <Plus className="h-4 w-4 mr-2" />
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <p className="text-sm">
                    {invoice.invoice_date ? formatDate(invoice.invoice_date) : 'Not set'}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {invoice.terms_days ? `Net ${invoice.terms_days} days` : 'Not specified'}
                  </p>
                </div>
                <div>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <p className="text-sm font-medium">
                    {invoice.customer?.business_name || invoice.customerName || 'Unknown Customer'}
                  </p>
                </div>
                {invoice.customer?.contact_person && (
                  <div>
                    <label className="text-sm font-medium text-muted-foreground">Contact Person</label>
                    <p className="text-sm">{invoice.customer.contact_person}</p>
                  </div>
                )}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium">{formatCurrency(displaySubtotal)}</span>
                </div>
                {displayTax > 0 && (
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">Tax</span>
                    <span className="text-sm font-medium">{formatCurrency(displayTax)}</span>
                  </div>
                )}
                {displayDiscount > 0 && (
                  <div className="flex justify-between">
                    <span className="text-sm text-muted-foreground">Discount</span>
                    <span className="text-sm font-medium text-red-600">-{formatCurrency(displayDiscount)}</span>
                  </div>
                )}
                <Separator />
                <div className="flex justify-between">
                  <span className="text-sm font-bold">Total</span>
                  <span className="text-sm font-bold">{formatCurrency(displayTotal)}</span>
                </div>
                {amountPaid > 0 && (
                  <>
                    <div className="flex justify-between">
