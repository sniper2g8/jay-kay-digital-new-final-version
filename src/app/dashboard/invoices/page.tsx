...
const getStatusIcon = (status: string) => {
  switch (status?.toLowerCase()) {
    ...
    case "sent": case "partial": return <Clock className="h-4 w-4" />;
    case "overdue": return <AlertCircle className="h-4 w-4" />;
    default: return <FileText className="h-4 w-4" />;
  }
};

const formatCurrency = (amount: number, currency: string = 'SLL'): string => {
      ...
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-yellow-600">{formatCurrency(stats.total_pending)}</div>
              <p className="text-xs text-muted-foreground">
