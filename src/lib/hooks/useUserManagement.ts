  ...
  try {
    ...
    const { error: profileError } = await supabase
      ...
      .update({
        ...
        updated_at: new Date().toISOString(),
        last_role_update: userData.primary_role
...
export async function getUsers(filters?: {
  ...
  try {
    ...
    if (filters?.search) {
      query = query.or(
        `name.ilike.%${filters.search}%,email.ilike.%${filters.search}%,human_id.ilike.%${filters.search}%`,
      );
    }
 ...
 * Validate user data
 */
export function validateUserData(userData: Partial<UserFormData>): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
...
export async function checkEmailExists(
  ...
  try {
    let query = supabase.from("appUsers").select("id").eq("email", email);
