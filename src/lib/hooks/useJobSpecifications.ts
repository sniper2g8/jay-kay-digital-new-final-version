...
type JobSpecification =
  Database["public"]["Tables"]["job_specifications"]["Row"];
...
export function usePaperSizes() {
  return useSWR<PaperSize[]>("paper-sizes", async () => {
    const { data, error } = await supabase
      .from("paper_sizes")
      .select("*")
      .order("name");
...
export function usePaperWeights() {
  return useSWR<PaperWeight[]>("paper-weights", async () => {
    const { data, error } = await supabase
      .from("paper_weights")
      .select("*")
      .order("gsm");
...
export function usePaperTypes() {
  return useSWR<PaperType[]>("paper-types", async () => {
    const { data, error } = await supabase
      .from("paper_types")
      .select("*")
      .order("name");
...
export function useFinishOptions() {
  return useSWR<FinishOption[]>("finish-options", async () => {
    try {
      const { data, error } = await supabase
        .from("finish_options")
        .select("*")
