...
export const useJobSubmissionForm = () => {
  ...
  const [availableFinishingOptions, setAvailableFinishingOptions] = useState<FinishOption[]>([]);
  const [finishingOptionPrices, setFinishingOptionPrices] = useState<Record<string, number>>({});

  const { paperSizes, paperWeights, paperTypes, finishOptions } = usePaperSpecifications();

  // Form steps configuration
  const formSteps = [
    { id: 'customer', title: 'Customer', description: 'Select or add customer' },
    { id: 'details', title: 'Job Details', description: 'Basic job information' },
    { id: 'specifications', title: 'Specifications', description: 'Size, paper & finishing' },
  ...
  }, [formData.unit_price, formData.finishing_options, formData.quantity, finishingOptionPrices]);
  ...
  const updateAvailableOptions = useCallback(() => {
    ...
    if (serviceSpec) {
      ...
      if (filteredPaperWeights.length > 0 && !filteredPaperWeights.includes(formData.paper_weight)) {
        setFormData(prev => ({ ...prev, paper_weight: filteredPaperWeights[0] }));
      }
    } else {
  ...
  const validateCurrentStep = (): boolean => {
    switch (currentStep) {
      ...
      case 1: // Job Details
        return !!(formData.service_id && formData.title && formData.unit_price > 0);
  ...
  // Update form data
  const updateFormData = (field: keyof JobFormData, value: JobFormData[keyof JobFormData]) => {
    setFormData(prev => ({ ...prev, [field]: value }));
  };

  const updateNewCustomerData = (field: keyof NewCustomerData, value: string) => {
