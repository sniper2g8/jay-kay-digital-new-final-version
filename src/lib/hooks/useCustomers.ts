  ...
  try {
    ...
    if (error) {
      ...
      if (
        ...
        console.error(
          "🔒 ROW LEVEL SECURITY ERROR: Anonymous users do not have permission to read customers table",
        );
        console.error(
          "📝 To fix this, you need to create RLS policies in your Supabase database",
        );
        console.error(
          "🛠️  Run the SQL in enable-anonymous-access.sql or implement proper authentication",
        );
        throw new Error(
          "Database access denied. Please configure Row Level Security policies or implement authentication.",
        );
      }
...
const fetchCustomerWithStats = async (
  customerHumanId: string,
  ...
  const lastPayment =
    (payments as { payment_date?: string }[])?.[0]?.payment_date || null;
    ...
    async () => {
      ...
      const customersWithStats = await Promise.all(
        customers.map(async (customer) => {
          try {
            const stats = await fetchCustomerWithStats(
              customer.human_id || customer.id,
            );
            return stats;
          } catch (error) {
            console.error(
              `Error fetching stats for ${customer.human_id || customer.id}:`,
              error,
            );
            return {
              ...customer,
