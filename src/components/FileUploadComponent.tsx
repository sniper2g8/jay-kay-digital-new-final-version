                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center space-x-2 text-xs text-gray-500">
                        ...
                        {upload.status === 'error' && upload.errorMessage && (
                          <>
                            <span>•</span>
                            <span className="text-red-600">{upload.errorMessage}</span>
                          </>
                        )}
                      </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2 text-xs">
              {hasCompletedUploads() && (
                <span className="px-2 py-1 bg-green-100 text-green-800 rounded">
                  {fileUploads.filter(f => f.status === 'completed').length} completed
                </span>
              )}
              {hasPendingUploads() && (
                <span className="px-2 py-1 bg-gray-100 text-gray-800 rounded">
                  {fileUploads.filter(f => f.status === 'pending').length} pending
                </span>
              )}
              {hasFailedUploads() && (
                <span className="px-2 py-1 bg-red-100 text-red-800 rounded">
                  {fileUploads.filter(f => f.status === 'error').length} failed
                </span>
              )}
            </div>
              ...
              <div className="mt-4 pt-4 border-t">
                ...
                >
                  {isUploading ? (
                    ...
                    <>
                      <Upload className="h-4 w-4 mr-2" />
                      Upload Files ({fileUploads.filter(f => f.status === 'pending').length})
                    </>
                  )}
                </Button>
              </div>
