...
export default function DatabaseConnectionTest() {
  ...
  const getStatusIcon = (status: DatabaseTest['status']) => {
    switch (status) {
      case 'pending': return <Loader2 className="h-4 w-4 animate-spin text-gray-400" />;
    ...
    <div className="max-w-2xl mx-auto p-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Testing connectivity to Supabase database and table access permissions
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            {errorCount > 0 && (
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                ...
                <div className="text-sm text-yellow-700 space-y-1">
                  <p>• Check if RLS (Row Level Security) policies allow anonymous access</p>
                  <p>• Verify that the NEXT_PUBLIC_SUPABASE_ANON_KEY is correct</p>
