...
async function fixJobsRLS() {
  ...
  try {
    ...
    for (const policy of existingPolicies.rows) {
      try {
        await client.query(
          `DROP POLICY IF EXISTS "${policy.policyname}" ON public.jobs`,
        );
        console.log(`  ✅ Dropped: ${policy.policyname}`);
      } catch (err) {
        console.log(
          `  ⚠️  Could not drop ${policy.policyname}: ${err.message}`,
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `✅ Jobs query successful! Found ${testResult.rows.length} jobs`,
      );
